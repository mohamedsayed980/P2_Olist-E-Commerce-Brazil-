...
# =============================================================================
## path = streamlit run "G:\FINAL PROJECTS\P2_Olist E-Commerce (Brazil)\ML_Models.py" 
# ─────────────────────────────────────────────────────────────────────────────
# A  IMPORTS
# ─────────────────────────────────────────────────────────────────────────────
# A1 — Core
...
def prepare_classification(df: pd.DataFrame):
    # ── For Olist: use is_satisfied (binary 0/1) directly
    feats   = S["feat_names"]
    df2     = df.copy()

    # Prefer is_satisfied; fall back to price binning for other datasets
    if "is_satisfied" in df2.columns:
...
with st.sidebar:
    ...
    if df_global is not None:
        ...
        target   = st.selectbox("Target column (regression)",
                                 num_cols,
                                 index=num_cols.index("delivery_days")
                                 if "delivery_days" in num_cols else 0)
        S["target_col"] = target
